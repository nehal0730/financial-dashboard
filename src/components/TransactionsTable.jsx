...
import { FaArrowTrendDown, FaArrowTrendUp, FaChevronDown, FaDownload, FaFilter, FaInbox, FaSortDown, FaSortUp } from 'react-icons/fa6';
      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="flex items-start justify-between gap-3 md:block">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white p-2.5 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-slate-400 md:hidden dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:border-slate-600"
            aria-label="Toggle transaction controls"
            aria-expanded={isMobileControlsOpen}
            aria-controls="mobile-transaction-controls"
          >
            <FaFilter size={12} />
          </button>
        </div>

        <div className="hidden flex-col gap-3 sm:flex-row sm:items-end md:flex">
        ...
        {isMobileControlsOpen ? (
          <div id="mobile-transaction-controls" className="space-y-3 md:hidden">
            <label className="flex flex-col text-xs font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400">
              <div className="px-1">Search</div>
              <input
                type="text"
            ...
            {hasAnyTransactions ? (
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => exportToCSV(filteredTransactions.length > 0 ? filteredTransactions : transactions)}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
                  title="Export as CSV"
                >
                  <FaDownload size={12} />
                  CSV
